...
def create_LLM_model(dt_train_named, dt_test_named, train_y, test_y, nr_events_all):
  ...
  return test_y_all, preds_all, coefficients_list, cls_list, cls_index, dt
...
def parsimony_GLRM(model_explainer):
  ...
  #event and case columns
  count_eventcase = len(model_explainer[(model_explainer['rule'].str.contains(filter))&(model_explainer['rule'].str.contains("static"))&(~model_explainer['rule'].str.contains("Activity"))])
  #case and control columns
  count_controlcase  = len(model_explainer[(model_explainer['rule'].str.contains("static"))&(model_explainer['rule'].str.contains("Activity"))])
  #event and control columns
  count_eventcontrol = len(model_explainer['rule'])-count_case-count_event-count_control-count_eventcase-count_controlcase
 
  parsimony_event = (count_event)/len_event
  parsimony_case = (count_case)/len_case 
  parsimony_control = (count_control)/len_control
  parsimony_eventcase = count_eventcase
  parsimony_controlcase = count_controlcase
  parsimony_eventcontrol =count_eventcontrol
  print('event attributes and percentage of event attributes:',count_event, parsimony_event)
  print('case attributes and percentage of case attributes:', count_case, parsimony_case)
  print('controlflow attributes and percentage of controlflow attributes', count_control, parsimony_control)
  print('eventcase attributes and percentage of eventcase attributes', count_eventcase, parsimony_eventcase)
  print('controlcase attributes and percentage of controlcase attributes', count_controlcase, parsimony_controlcase)
  print('eventcontrol attributes and percentage of evencontrol attributes', count_eventcontrol, parsimony_eventcontrol)
 
  return parsimony_event, parsimony_case, parsimony_control, parsimony_eventcase, parsimony_controlcase,parsimony_eventcontrol, count_eventcase, count_controlcase, count_eventcontrol
...
results_dir ='./results_dir_ML_mono'
    ...
    for cls_method in classifiers:
        for cls_encoding in encoding:
            ...
            if cls_method =='LR':
                ...
                test_y_all, preds_all, coefficients_list, cls_list, cls_index, dt = create_LLM_model(dt_train_named, dt_test_named, train_y, test_y, nr_events_all)
                #auc total
                auc_total = roc_auc_score(test_y_all, preds_all)
                ...
                #fit model
                cls= LogisticRuleRegression(lambda0=args['lambda0'], lambda1=args['lambda1'], iterMax= 1000)   
                ...
                #parsimony
               # parsimony_event, parsimony_case, parsimony_control, parsimony_eventcase, parsimony_controlcase,parsimony_eventcontrol, len_eventcase, len_controlcase, len_eventcontrol = parsimony_GLRM(cls.explain())
            ...
            with open(outfile, 'w') as fout:
               ...
               #if cls_method=='GLRM':
#                        fout.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n"%("parsimony controlcase",parsimony_controlcase,"parsimony eventcase", parsimony_eventcase,"parsimony_eventcontrol", parsimony_eventcontrol, "len_eventcase",len_eventcase,"len_controlcase",len_controlcase,"len_eventcontrol", len_eventcontrol))
