...
class IndexBasedTransformer(TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if self.max_events is None:
            self.max_events = ['size',pd.DataFrame(grouped.size())['size'].max()]
        
        
        dt_transformed = pd.DataFrame(grouped.apply(lambda x: x.name), columns=[self.case_id_col])

        for i in range(self.max_events[1]):
        ...
        if self.create_dummies:
            all_cat_cols = ["%s_%s"%(col, i) for col in self.cat_cols for i in range(self.max_events[1])]
